...
namespace esphome
{
    namespace artnet_dmx
    {
        ...
        void ArtNetDMXOutput::setup()
        {
            if (this->dere_pin_)
            {
                ...
                this->dere_pin_->digital_write(true); // TX aktiv: DE=HIGH, /RE=HIGH
            }
    ...
    if (n > 18 && std::memcmp(d, "Art-Net", 7) == 0 && d[8] == 0x00 && d[9] == 0x50) {  // OpCode ArtDMX
      ...
      if (incoming_universe == this->universe_) {
        ...
        void ArtNetDMXOutput::send_dmx(uint16_t length)
        {
            if (!this->uart_)
                return;
            this->uart_->write_byte(0x00); // Startcode
